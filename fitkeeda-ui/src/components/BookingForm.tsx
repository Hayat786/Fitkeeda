...
import api, { createBooking, getAllSessions, SessionData } from "@/utils/api";
...
export default function BookingForm() {
  ...
  // Scroll to form section on step change (mobile fix)
  useEffect(() => {
    if (formRef.current) {
  ...
  // Fetch resident info and sessions
  useEffect(() => {
  ...
  // Update slots when sport changes
  useEffect(() => {
    if (formData.sport) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const bookingPayload = {
        ...formData,
        residentId: decoded.id, // now this is in scope
      };
  ...
  return (
    <div className="min-h-screen lg:h-screen gradient-bg flex flex-col lg:flex-row items-center justify-center p-6 gap-8 lg:gap-12">
      ...
      {/* Left: Logo + Motivating Message */}
     
<motion.div
  className="flex flex-col justify-center items-center text-center lg:text-left lg:w-1/3 p-4 lg:p-6
             lg:h-screen lg:max-h-screen gap-4 lg:gap-6"
    ...
    >
      ...
      <motion.div
        ...
        className="flex flex-col justify-between backdrop-blur-xl bg-black/5 p-6 lg:p-8 rounded-2xl shadow-2xl w-full max-w-lg border border-black/10
                   lg:h-[90%] lg:max-h-screen overflow-auto"
        initial={{ opacity: 0, x: 50 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.8 }}
      >
          ...
          <>
            ...
            <div className="space-y-4">
              {[
                ...
                { name: "apartment", placeholder: "Apartment / Society", icon: FaFutbol, type: "text" },
                { name: "number", placeholder: "Phone Number", icon: FaPhone, type: "text" },
              ].map((field, idx) => {
                ...
                return (
                  <div key={idx} className="relative">
                    ...
                    <input
                      ...
                      className="w-full p-3 pl-10 rounded-lg bg-white text-black placeholder-gray-400
            ...
            <button
              onClick={() => setStep(2)}
              className="mt-6 w-full flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white py-3 rounded-xl font-semibold shadow-lg transition-all"
            >
        ...
        {step === 2 && (
          <>
                ...
                return (
                  <div key={idx} className="relative">
                    ...
                    <select
                      ...
                      onChange={handleChange}
                      className="w-full p-3 pl-10 rounded-lg bg-white text-black placeholder-gray-400
                                 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all appearance-none
                                 overflow-auto max-h-[200px] lg:max-h-full"
                    >
            ...
            <div className="flex justify-between mt-6">
              <button
                onClick={() => setStep(1)}
                className="px-6 py-3 bg-gray-300 text-black rounded-xl hover:bg-gray-400 transition-all flex items-center gap-2"
              >
                <FaArrowLeft /> Back
              </button>
              <button
                onClick={handleSubmit}
                className="px-6 py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-semibold shadow-lg transition-all flex items-center gap-2"
              >
                <FaCheck /> Submit
              </button>
            </div>
