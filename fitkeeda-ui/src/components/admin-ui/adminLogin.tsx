...
import { adminLogin, AdminLoginData } from "@/utils/api";
...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const data: AdminLoginData = { phone, password };
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row gradient-bg items-center justify-center px-6">
      ...
      {/* Desktop Logo */}
      <motion.div
        className="hidden lg:flex w-1/2 relative h-[400px]"
        ...
        <motion.div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
          ...
          <form className="space-y-5" onSubmit={handleLogin}>
            ...
            <motion.button
              ...
              type="submit"
              disabled={submitting}
              className={`w-full py-3 bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-lg font-semibold shadow hover:opacity-90 transition disabled:opacity-60 ${sourceSans.className}`}
            >
